...
<script type="text/javascript">
	...
	$(function(){
		$(".btn").on('click',getExtent);
		initVal();
		var dialog = parent.art.dialog.list["bookmarkedit"];
		var dialog_div = dialog.DOM.wrap;
		dialog_div.on("ok", function() {
			if(!validate()) return;
			var option = {
					url : "${path}/locateService/edit.do",
					cache : false,
					dataType : 'json',
					type:'post',
...
<body>
<form action="${path}/locateService/add.do" method="post" id="form_id">
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="date_add_table">
  <tr>
    <td width="120" class="t_r">书签名称：</td>
    <td ><input type="text" name="name" id="name" width="163px" class="text" value="${o.name}" /></td>
  </tr>
  <tr>
    <td class="t_r">书签描述：</td>
    <td><textarea name="description" id="" cols="28" rows="3" class="text_wid_area">${o.description}</textarea></td>
  </tr>
  <tr>
...
<input type="hidden" name="creator" id="creator" value="${o.creator}">
<input type="hidden" name="createtime" id="createtime" value="${o.createTime}">
<input type="hidden" name="xmin" id="xmin" value="${o.xmin}">
<input type="hidden" name="ymin" id="ymin" value="${o.ymin}">
<input type="hidden" name="xmax" id="xmax" value="${o.xmax}">
<input type="hidden" name="ymax" id="ymax" value="${o.ymax}">
</form>
</body>
