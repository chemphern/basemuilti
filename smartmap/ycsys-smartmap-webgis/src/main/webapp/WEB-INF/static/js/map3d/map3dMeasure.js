...
var MeasureTool={
    ...
    MeasureType:{'HORIZONTAL':'HORIZONTAL','VERTICAL':'VERTICAL','AERICAL':'AERICAL','GROUNDLINE':'GROUNDLINE','AREA':'AREA','GROUNDAREA':'GROUNDAREA'},
    deactivate:function(){
...
function measureToolLBUpHandler(flags, x, y) {
    ...
    var tempOperation=MeasureToolGlobe.MeasureOperation;
       ...
       if(tempOperation==MeasureTool.MeasureType.AERICAL){//测量空间距离
        ...
        if(tempOperation==MeasureTool.MeasureType.GROUNDLINE){//测量空间距离
            measureGroundLineByLBUp(clickPosition);
        }else if(tempOperation=="ADDGROUNDLINE"){
            addGroundLinePointByLUp(clickPosition);
        }
...
function measureToolRBUpHandler(){
	//结束量测操作
	if(MeasureToolGlobe.MeasureOperation=="ADDAERIAL"){           
        addAerialPointByRUp();
    }else if(MeasureToolGlobe.MeasureOperation=="ADDGROUNDLINE"){
        addGroundLinePointByRUp();
    }else if(MeasureToolGlobe.MeasureOperation=="ADDAREA"){
        addAreaPointByRUp();
    }else if(MeasureToolGlobe.MeasureOperation=="ADDGROUNDAREA"){
...
function measureToolFrameHandler(){       
    ...
    if(mousePosition.X && mousePosition.Y){
	    if(MeasureToolGlobe.MeasureOperation=="ADDHORIZONTAL"||MeasureToolGlobe.MeasureOperation=="ADDVERTICAL"){   
            ...
            addGroundLinePointFrame(mousePosition);
        }
	    else if(MeasureToolGlobe.MeasureOperation=="ADDAREA"){
...
function measureGroundLineByLBUp(clickPosition) {
    clickPosition.AltitudeType=2;
    clickPosition.Altitude = 0;
    //先创建一个包含起点和终点的拆线及起点Label终点Label
    var geoArr = new Array(clickPosition.x, clickPosition.y, 0, clickPosition.x, clickPosition.y, 0);
    YcMap3D.Creator.CreatePolylineFromArray(geoArr,MeasureToolGlobe.LineColor , 2, MeasureToolGlobe.MeasureFolderID, 'GroundLine');
    //创建起点标识
    YcMap3D.Creator.CreateTextLabel(clickPosition,"起点",MeasureToolGlobe.LabelStyle,MeasureToolGlobe.MeasureFolderID,"GroundLineResultLabel");
    YcMap3D.Creator.CreateImageLabel(clickPosition,MeasureToolGlobe.ImagePath,MeasureToolGlobe.LabelStyle,MeasureToolGlobe.MeasureFolderID,"GroundLineStartLabel");
    YcMap3D.Creator.CreateImageLabel(clickPosition,MeasureToolGlobe.ImagePath,MeasureToolGlobe.LabelStyle,MeasureToolGlobe.MeasureFolderID,"GroundLineMoveEndLabel");
    var resultlabel=YcMap3D.Creator.CreateTextLabel(clickPosition,"0米",MeasureToolGlobe.LabelStyle,MeasureToolGlobe.MeasureFolderID,"GroundLineMoveResultLabel");
    MeasureToolGlobe.MeasureOperation="ADDGROUNDLINE";
    resultlabel.Style.PivotAlignment='Bottom,Center';
}

function addGroundLinePointFrame(mousePosition) {
    var aerial=YcMap3D.ProjectTree.GetObject(YcMap3D.ProjectTree.FindItem(MeasureToolGlobe.MeasureFolder+"\\GroundLine"));
    var startPoint=aerial.Geometry.StartPoint;
    mousePosition.AltitudeType=2;
    mousePosition.Altitude = 0;
    //设置终点位置
    aerial.Geometry.StartEdit();
    aerial.Geometry.EndPoint.X=mousePosition.X;
    aerial.Geometry.EndPoint.Y=mousePosition.Y;
    aerial.Geometry.EndPoint.Z=0;
    aerial.Geometry.EndEdit();
    //设置终点的Label位置
    var endLabel=YcMap3D.ProjectTree.GetObject(YcMap3D.ProjectTree.FindItem(MeasureToolGlobe.MeasureFolder+"\\GroundLineMoveEndLabel"));
    endLabel.Position=mousePosition;
    //显示结果距离及设置结果Label位置
    var lastSecondPoint= aerial.Geometry.Points.Item(aerial.Geometry.Points.Count-2);
    var lastSecondPosition=YcMap3D.Creator.CreatePosition(lastSecondPoint.X,lastSecondPoint.Y,0,2);
    var geoArra = new Array(lastSecondPoint.x, lastSecondPoint.y, 0, mousePosition.x, mousePosition.y, 0);
    var lineGeo = YcMap3D.Creator.GeometryCreator.CreateLineStringGeometry(geoArra);
    var distance= YcMap3D.Analysis.MeasureTerrainGroundDistance(lineGeo,0.5,false);
    var resultLabel=YcMap3D.ProjectTree.GetObject(YcMap3D.ProjectTree.FindItem(MeasureToolGlobe.MeasureFolder+"\\GroundLineMoveResultLabel"));
    resultLabel.Text=distance.toFixed(2)+"米";
    resultLabel.Position.X=(lastSecondPosition.X+mousePosition.X)*0.5;
    resultLabel.Position.Y=(lastSecondPosition.Y+mousePosition.Y)*0.5;
    resultLabel.Position.AltitudeType = 2;
    resultLabel.Position.Altitude=0;
}

function addGroundLinePointByLUp(clickPosition) {
    clickPosition.AltitudeType=2;
    clickPosition.Altitude = 0;
    //点击时添加一个终点及一个结果Label、转折点Label
    var aerial=YcMap3D.ProjectTree.GetObject(YcMap3D.ProjectTree.FindItem(MeasureToolGlobe.MeasureFolder+"\\GroundLine"));
    aerial.Geometry.StartEdit();
    aerial.Geometry.Points.AddPoint(clickPosition.X,clickPosition.Y,0);
    aerial.Geometry.EndEdit();
    YcMap3D.Creator.CreateImageLabel(clickPosition,MeasureToolGlobe.ImagePath,MeasureToolGlobe.LabelStyle,MeasureToolGlobe.MeasureFolderID,"GroundLineLabel");
    var resultLabel=YcMap3D.ProjectTree.GetObject(YcMap3D.ProjectTree.FindItem(MeasureToolGlobe.MeasureFolder+"\\GroundLineMoveResultLabel"));
    YcMap3D.Creator.CreateTextLabel(resultLabel.Position,resultLabel.Text,resultLabel.Style,MeasureToolGlobe.MeasureFolderID,"GroundLineResultLabel");
}

function addGroundLinePointByRUp() {
    var aerial=YcMap3D.ProjectTree.GetObject(YcMap3D.ProjectTree.FindItem(MeasureToolGlobe.MeasureFolder+"\\GroundLine"));
    aerial.Geometry.StartEdit();
    aerial.Geometry.Points.DeletePoint(aerial.Geometry.Points.Count-1);
    aerial.Geometry.EndEdit();
    YcMap3D.Window.SetInputMode(0);
    //标识终点及总长度
    var resultText = "总长度：" +  YcMap3D.Analysis.MeasureTerrainGroundDistance(aerial.Geometry,0.5,false).toFixed(2) + "米";
    var aerialEndPosition = YcMap3D.Creator.CreatePosition(aerial.Geometry.EndPoint.X,aerial.Geometry.EndPoint.Y,0,2);
    YcMap3D.Creator.CreateTextLabel(aerialEndPosition,resultText,MeasureToolGlobe.LabelStyle,MeasureToolGlobe.MeasureFolderID,"GroundLineResultLabel");
    //删除过程元素
    YcMap3D.ProjectTree.DeleteItem(YcMap3D.ProjectTree.FindItem(MeasureToolGlobe.MeasureFolder+"\\GroundLineMoveResultLabel"));
    YcMap3D.ProjectTree.DeleteItem(YcMap3D.ProjectTree.FindItem(MeasureToolGlobe.MeasureFolder+"\\GroundLineMoveEndLabel"));
    MeasureToolGlobe.MeasureOperation="EndMeasure";
}
