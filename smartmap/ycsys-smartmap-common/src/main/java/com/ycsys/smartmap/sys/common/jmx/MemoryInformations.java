...
public class MemoryInformations implements Serializable {
	...
	static {
		try {
			NIO_BUFFER_POOLS.addAll(new com.ycsys.smartmap.sys.common.jmx.MBeans().getNioBufferPools());
	...
	private static long getUsedBufferMemory() {
		...
		long result = 0;
		final com.ycsys.smartmap.sys.common.jmx.MBeans mBeans = new com.ycsys.smartmap.sys.common.jmx.MBeans();
		try {
	...
	private String buildMemoryDetails() {
		final DecimalFormat integerFormat = new DecimalFormat("#,##0",  DecimalFormatSymbols.getInstance());
