...
<body>
<form action="${ctx}/dictionary/updateItem" id="dicItemForm" name="dicItemForm" method="post" ligeruiid="dicItemForm">
    <input name="id" id="id" value="${item.id}" type="hidden">
    <input name="dictionary.id" value="${item.dictionary.id}" type="hidden">
    <table heigth="100%" class="l-table-edit" border="0" cellpadding="0" cellspacing="0" class="date_add_table">
        ...
        <tr style="line-height:40px;">
            ...
            <td width="160px">
                <div class="l-text" style="width: 180px;">
                    <input name="name" id="itemName" validate="{required:true}" value="${item.name}" class="l-text-field" style="width: 176px;" ligeruiid="itemName" type="text">
                </div>
            </td>
            <td width="20px"></td>
            <td width="60px">值：</td>
            <td width="160px">
                <div class="l-text" style="width: 180px;">
                    <input name="value" id="itemValue" validate="{required:true}" value="${item.value}" class="l-text-field" style="width: 176px;" ligeruiid="itemValue" type="text">
                </div></td>
            <td></td>
        </tr>
        <tr style="line-height:40px;">
            ...
            <td>
                <div class="l-text" style="width: 180px;">
                    <input name="sort" id="showOrder" validate="{required:true}" value="${item.sort}" class="l-text-field" style="width: 176px;" ligeruiid="showOrder" type="text">
                </div>
            </td>
            <td></td>
            <td>是否显示：</td>
            <td>
                <select name="isShow">
                    <option value="1" <c:if test="${item.isShow eq 1}">selected="true"</c:if>>显示</option>
                    <option value="2" <c:if test="${item.isShow eq 2}">selected="true"</c:if>>不显示</option>
                </select>
            </td>
            <td>
            </td></tr>
        <tr style="border-bottom:1px solid #EBEBEB;line-height:40px;">
            <td>备注：</td>
            <td colspan="4">
                <textarea cols="78" rows="8" id="memo" name="memo" class="l-textarea">${item.memo}</textarea>
            </td>
            <td>
...
<script>
    function yc_close(){
        var p = window.parent;
        var dialog = p.art.dialog.list["updateDictItemDialog"];
        dialog.close();
    }
    $(function(){
        var p = window.parent;
