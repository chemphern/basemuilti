...
<head>
    ...
    <title>羽辰智慧林业平台运维管理系统-角色管理修改</title>
...
<form action="${ctx}/role/saveOrUpdate" method="post" id="form_id">
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="date_add_table">
        ...
        <tr>
            ...
            <td>
                <input type="text" style="display: none" name="id" value="${role.id}"/>
                <input type="text" name="name" id="name" validate="{required:true,messages:{required:'必填'}}" value="${role.name}"/>
            </td>
        </tr>
        <tr>
            <td class="t_r">角色编码：</td>
            <td><input type="text" name="code" id="code" validate="{required:true,messages:{required:'必填'}}" value="${role.code}"/></td>
        </tr>
        <tr>
            <td class="t_r">备注：</td>
            <td><textarea cols="30" rows="5" id="remark" name="remark">${role.remark}</textarea></td>
        </tr>
